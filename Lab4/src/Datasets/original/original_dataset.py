...
class OriginalDataset:
    ...
    def from_cropped_to_dataset(self) -> dict[int, int]:
        ...
        ids_count = {}
        total_images = 0
        annotations_file = open(f"{self.annotations_path}/expanded_annotations.txt", "w")
        
        for directory in os.listdir(self.cropped_images_path):
            if directory not in OriginalDataset.celebrity_to_id:
                print(f"Unknown celebrity name. Skipping directory: {directory}")
            else:
                curr_id = OriginalDataset.celebrity_to_id[directory]
    ...
    def __get_ids_count(self, path: str):
        ...
        for directory in sorted(os.listdir(path)):
            if directory in OriginalDataset.celebrity_to_id:
                curr_id = OriginalDataset.celebrity_to_id[directory]
                ids_count[curr_id] = 0
